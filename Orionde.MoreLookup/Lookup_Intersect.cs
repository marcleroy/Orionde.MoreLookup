...
namespace Orionde.MoreLookup
{
    public static partial class LookupExtensions
    {
        ...
        /// </param>
        /// <returns>
        ///     An <see cref="ILookup{TKey, TValue}" /> that contains the elements that form the set intersection of the two
        ///     input lookups.
        /// </returns>
            ...
            IEqualityComparer<TKey> keyComparer, IEqualityComparer<TValue> valueComparer)
        {
            ...
            return firstKeys.Where(x => secondKeys.Remove(x))
                .Select(x =>
                    Intersect_ValuesForKey(x, first.ValuesForKey(x, keyComparer), second, keyComparer, valueComparer));
        }
