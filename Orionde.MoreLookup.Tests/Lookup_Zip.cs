...
public class LookupZipTests
{
    ...
    public void When_zipping_lookups_should_zip_according_to_selector()
    {
        ...
        // Act
        ILookup<int, string>? zipped = lookup.Zip(Lookup.LookupBuilder.WithKey(2, new[] { "e", "d" })
    ...
    {
        // Arrange
        ILookup<string, int>? lookup = Lookup.LookupBuilder.WithKey("one", new[] { 1, 2 })
            .WithKey("ONE", new[] { 3 }).Build();

        // Act
        ILookup<string, string>? zipped = lookup.Zip(Lookup.LookupBuilder.WithKey("two", new[] { "a", "b" })
    ...
    public void When_zipping_null_with_lookup_should_throw_ArgumentNullException()
    {
        ...
        // Act & Assert
        ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => lookup.Zip(Lookup.LookupBuilder.WithKey(2, new[] { "e", "d" })
            .WithKey(3, new[] { "f", "g" }).Build(), (x, y) => x + y));
    ...
    public void When_zipping_lookup_with_null_should_throw_ArgumentNullException()
    {
        ...
        ArgumentNullException exception =
            Assert.Throws<ArgumentNullException>(() => lookup.Zip<int, string, int, string>(null, (x, y) => x + y));
        exception.Should().BeOfType<ArgumentNullException>();
    }
